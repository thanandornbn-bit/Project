...
<head>
    ...
    <style>
        * {
        ...
        .stat-total::before { background: linear-gradient(90deg, #ff8c00, #ff6b00); }
        .stat-total .stat-icon, .stat-total .stat-number { color: #ff8c00; }

        .stat-available::before { background: linear-gradient(90deg, #00ff88, #00cc6f); }
        .stat-available .stat-icon, .stat-available .stat-number { color: #00ff88; }

        .stat-occupied::before { background: linear-gradient(90deg, #ff4444, #cc0000); }
        .stat-occupied .stat-icon, .stat-occupied .stat-number { color: #ff4444; }

        .stat-pending::before { background: linear-gradient(90deg, #ffa726, #f57c00); }
        .stat-pending .stat-icon, .stat-pending .stat-number { color: #ffa726; }

        .stat-revenue::before { background: linear-gradient(90deg, #42a5f5, #1976d2); }
        .stat-revenue .stat-icon, .stat-revenue .stat-number { color: #42a5f5; }
        ...
        @media (max-width: 768px) {
            .header { font-size: 1.8rem; padding: 20px; }
        ...
        <div class="stats-container">
            ...
            <div class="stat-card stat-pending">
                ...
                <div class="stat-number">
                    ...
                    <c:forEach var="room" items="${roomList}">
                        <c:if test="${room.roomStatus == 'ไม่ว่าง' && roomDepositStatus[room.roomID] == 'รอดำเนินการ'}">
            ...
            <div class="stat-card stat-revenue">
                ...
                <div class="stat-number">
                    ...
                    <c:forEach var="room" items="${roomList}">
                        <c:if test="${room.roomStatus == 'ไม่ว่าง'}">
                            <c:set var="totalRevenue" value="${totalRevenue + room.roomPrice}" />
                        </c:if>
                    </c:forEach>
        ...
        <div class="nav">
            ...
            <a href="OViewReserve">
                ...
                <c:if test="${pendingCount > 0}">
                    <span class="admin-badge" style="margin-left: 5px; font-size: 0.7rem;">${pendingCount}</span>
                </c:if>
            </a>
        </div>
            ...
            <c:choose>
                <c:when test="${empty roomList}">
                    <div class="empty-state">
                        ...
                        <h3>ไม่พบห้องพัก</h3>
                        <p>ไม่มีห้องพักที่ตรงกับเงื่อนไขการค้นหา กรุณาลองเปลี่ยนตัวกรองหรือเพิ่มห้องใหม่</p>
                    </div>
                </c:when>
                <c:otherwise>
                    <table>
                        ...
                        <tbody>
                            <c:forEach var="room" items="${roomList}">
                                <tr>
                                    ...
                                    <td class="price">฿<fmt:formatNumber value="${room.roomPrice}" groupingUsed="true" /></td>
                                    <td>
                                        ...
                                        <c:if test="${room.roomStatus == 'ไม่ว่าง'}">
                                            <br>
                                            <small class="deposit-status ${roomDepositStatus[room.roomID] == 'เสร็จสมบูรณ์' ? 'approved' : 'pending'}">
                                                <i class="fas ${roomDepositStatus[room.roomID] == 'เสร็จสมบูรณ์' ? 'fa-check-circle' : 'fa-clock'}"></i>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <c:if test="${room.roomStatus == 'ไม่ว่าง'}">
                                                <c:choose>
                                                    <c:when test="${roomApprovalStatus[room.roomID] == true}">
                                                        <a href="ManagerAddInvoice?roomID=${room.roomID}" class="action-btn btn-add" 
                                                           title="เพิ่มใบแจ้งหนี้สำหรับห้อง ${room.roomNumber}">
                                                            <i class="fas fa-plus"></i>
                                                            เพิ่มบิล
                                                        </a>
                                                        <a href="EditInvoice?roomID=${room.roomID}" class="action-btn btn-edit-invoice" 
                                                    ...
                                                    <c:otherwise>
                                                        ...
                                                        <a href="OViewReserve" class="action-btn btn-view" 
                                            ...
                                            <a href="editRoom?id=${room.roomID}" class="action-btn btn-edit" 
